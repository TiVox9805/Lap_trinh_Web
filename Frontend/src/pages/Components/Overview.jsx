        ...
        <div className="flex flex-col gap-8 p-2 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/60 backdrop-blur-md p-6 rounded-[2rem] border border-white shadow-xl shadow-slate-200/50">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Dữ liệu cập nhật thời gian thực từ khoa {userObj.ten_khoa}</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-5 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between">
                    <div>
                        <h3 className="text-lg font-bold text-slate-800 mb-1">Công suất giường bệnh từ khoa {userObj.ten_khoa}</h3>
                ...
                <div className="lg:col-span-7 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-sm text-slate-400">Phân bổ theo từng chuyên khoa</p>
                        </div>
                        <button className="text-xs font-bold text-blue-600 hover:underline">Xem chi tiết</button>
                    </div>

                    <div className="grid gap-2">
                        {data.departments && data.departments.map((dept, index) => {
                            const percentage = data.patients.total > 0
                                ? ((dept.count / data.patients.total) * 100).toFixed(0)
                                : 0;
